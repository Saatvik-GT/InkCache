...
- Consistent hashing + a cluster gateway (`npm run start:gateway`, `INKCACHE_CLUSTER_NODES`): shards keys across a set of nodes so a client can talk to one address instead of knowing which node owns which key, with active health checking that pulls a dead node out of rotation (and back in once it recovers), node discovery via self-registration to **multiple** gateways at once (`INKCACHE_GATEWAY_URL` accepts a comma-separated list), and gateway-to-gateway gossip sync (`INKCACHE_PEER_GATEWAYS`) so a node registered with only one gateway is still routable through its peers — see [docs/api.md#cluster-gateway](docs/api.md#cluster-gateway)
...
- Opt-in shared-secret authentication (`INKCACHE_API_KEY`, constant-time comparison, propagated automatically through replication/gateway-proxying/self-registration so the whole cluster keeps working once it's set everywhere) and per-process rate limiting (`INKCACHE_RATE_LIMIT`) — see [docs/api.md#authentication--rate-limiting](docs/api.md#authentication--rate-limiting)
- Primary promotion: manual (`POST /promote`, one API call instead of a restart) and automatic (`INKCACHE_AUTO_PROMOTE=true`, a replica campaigns for a real majority vote from its `INKCACHE_PEER_URLS` before self-promoting — safe for **any number of replicas**, not just one, via a real Raft-style term/vote election; verified with a 3-replica test asserting exactly one winner) — see [docs/api.md#automatic-primary-promotion](docs/api.md#automatic-primary-promotion)

**Not yet implemented (roadmap):** auth has no per-client keys, expiry, or rotation — one shared secret for the whole cluster, real for a single-trusted-operator scope and no further. The dashboard doesn't send an API key yet, so enabling auth breaks it against that node until it's updated. The "adaptive intelligence layer" in the architecture diagram is still aspirational as an actual _trained/learned_ model — what exists today (access-aware eviction, and the access predictor above) is real statistical engineering, deliberately not machine learning. Nothing in the dashboard is mocked — every number comes from the running node.
...
- [x] Sprint 4: Consistent hashing (`src/core/hashring.ts`), a cluster
      gateway that shards keys across a node set (`npm run start:gateway`),
      failure handling (active health checks pull a dead node out of the
      hashing ring and back in once it recovers), and node discovery — a
      node can self-register with a running gateway at startup
      (`INKCACHE_GATEWAY_URL` + `INKCACHE_SELF_URL`) and deregisters on a
      graceful shutdown, instead of the gateway's node set being fixed at
      its own startup. See
      [docs/api.md#cluster-gateway](docs/api.md#cluster-gateway). Not a
...
| POST   | `/promote`            | Flips a replica to a primary — manual or automatic     |
