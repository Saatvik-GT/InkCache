...
- ASCII-terminal dashboard (React + Vite + Tailwind + react-router-dom), two routes:
  - `/` — a home page whose hero is a **rotating ASCII moon**: a real 3D sphere (spherical sampling, per-cell depth buffer, diffuse lighting against a fixed world-space light, limb darkening, and an object-space crater field) rasterized to characters instead of pixels, with rotation speed driven by live ops/sec. Headlines are a hand-built 5×7 bitmap font. Below it: a live node readout as a dot-leader manifest, feature panels, a quick-start snippet, and a `[x]`/`[ ]` roadmap checklist
  - `/dashboard` — the console: keyboard-driven KV console (`set k v [ttl]`, `get k`, `del k`, `flush`), block-glyph meters and sparklines for hit rate / ops-per-sec / p95 latency, a KEYS access-frequency heat map using shade glyphs (`░▒▓█`) sorted hottest-first, a colour-coded op stream where every line also carries its kind as text, optional synthesized sound cues (Web Audio, off by default), a glyph + label node status, synthetic traffic simulator (fires real requests), and a POST-style boot screen
  - No WebGL anywhere — the sphere is pure math over a character grid, which is why the whole dashboard ships in a single ~267 KB bundle. Both routes respect `prefers-reduced-motion` (the moon holds a static lit frame rather than just slowing down)
