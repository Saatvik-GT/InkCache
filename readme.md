...
- Primary promotion: manual (`POST /promote`, one API call instead of a restart) and automatic (`INKCACHE_AUTO_PROMOTE=true`, a replica self-promotes after its primary fails repeated liveness checks — **single-replica topologies only**, see below) — see [docs/api.md#automatic-primary-promotion](docs/api.md#automatic-primary-promotion)

**Not yet implemented (roadmap):** automatic promotion is only safe with exactly one replica per primary — with two or more replicas independently watching the same primary, enabling it on more than one risks split-brain (both self-promoting at once), since this project has no leader election/quorum to guarantee at most one winner, and no registry of sibling replicas a node could even check. Auth has no per-client keys, expiry, or rotation — one shared secret for the whole cluster, real for a single-trusted-operator scope and no further. The dashboard doesn't send an API key yet, so enabling auth breaks it against that node until it's updated. The "adaptive intelligence layer" in the architecture diagram is still aspirational as an actual _trained/learned_ model — what exists today (access-aware eviction, and the access predictor above) is real statistical engineering, deliberately not machine learning. Nothing in the dashboard is mocked — every number comes from the running node.
